  ...
  const slides = [
    ...
    <Slide key={0}>
      ...
      <div style={{ position:"relative", zIndex:2, flex:1, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", textAlign:"center", padding:"0 4rem" }}>
        ...
        <h1 style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:800, fontSize:"clamp(2.75rem, 4.5vw, 3.4rem)", lineHeight:1.08, letterSpacing:"-0.04em", color:"var(--c-text)", margin:"0 0 1.5rem", maxWidth:960 }}>
          {T.hero.headline1}<br />
          {T.hero.headline2}<br />
          <GoldText>{T.hero.headlineAccent.replace("\n", " ")}</GoldText>
        </h1>

        <p style={{ fontSize:"1.55rem", color:"var(--c-text2)", maxWidth:680, lineHeight:1.55, marginBottom:"3rem" }}>
          {T.hero.tagline}
        </p>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:"1px", background:"rgba(0,0,0,0.07)", borderRadius:20, overflow:"hidden", width:"100%", maxWidth:760 }}>
          {T.stats.map((s, i) => (
            <div key={i} style={{ background:"rgba(255,255,255,0.6)", padding:"1.25rem 1rem", textAlign:"center" }}>
              <div style={{ fontWeight:800, fontSize:"2.4rem", letterSpacing:"-0.03em", background:"linear-gradient(135deg, var(--c-gold3) 0%, var(--c-gold) 50%, var(--c-gold2) 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>
                {[7,1,35,60][i]}<span style={{ fontSize:"1.4rem" }}>{s.suffix}</span>
              </div>
              <div style={{ fontSize:"1rem", color:"var(--c-text3)", marginTop:6 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    /* 1 — ПРОБЛЕМА */
    <Slide key={1}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        <SlideLabel>{P.sectionLabel}</SlideLabel>
        <SlideTitle size="2.9rem">
          {P.title1}<br />
          <span style={{ color:"var(--c-red)" }}>{P.title2}</span>
        </SlideTitle>

        <p style={{ fontSize:"1.5rem", color:"var(--c-text2)", lineHeight:1.55, marginBottom:"1.5rem", maxWidth:780 }}>
          {P.lead}
        </p>
          ...
          {P.items.map((item, i) => (
            <Card key={i} red>
              <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
                <span style={{ fontSize:14, fontWeight:700, color:"var(--c-red)", opacity:0.7, minWidth:22 }}>0{i+1}</span>
                <span style={{ fontSize:"1.5rem", fontWeight:600, color:"var(--c-text)", lineHeight:1.35 }}>{item}</span>
              </div>
            </Card>
          ))}
        </div>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", gap:"0.85rem", padding:"1rem 1.25rem", borderRadius:14, background:"rgba(217,48,37,0.06)", border:"1.5px solid rgba(217,48,37,0.25)" }}>
          ...
          <span style={{ fontSize:"1.5rem", color:"var(--c-text)", lineHeight:1.5 }}>
    ...
    <Slide key={2}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <SlideTitle size="2.9rem">Кто выигрывает от наличия<br />собственного производства?</SlideTitle>

        <div style={{ display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:"0.85rem", flex:1, maxHeight:"52vh" }}>
          {P.ambitions.map((a, i) => (
            <Card key={i} gold style={{ display:"flex", flexDirection:"column", gap:"0.6rem" }}>
              <div style={{ fontSize:13, fontWeight:700, color:"var(--c-gold)", opacity:0.5, letterSpacing:"0.08em" }}>{a.num}</div>
              <span style={{ fontSize:"2rem", lineHeight:1 }}>{a.emoji}</span>
              <div style={{ fontSize:"1.5rem", fontWeight:700, color:"var(--c-text)", lineHeight:1.35 }}>{a.title}</div>
              <div style={{ fontSize:"1.2rem", color:"var(--c-text2)", lineHeight:1.45 }}>{a.desc}</div>
            </Card>
          ))}
        </div>
      </div>
    ...
    <Slide key={3}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <div style={{ display:"flex", gap:"0.85rem", marginBottom:"1.1rem" }}>
          <Card red style={{ flex:1 }}>
            <div style={{ fontSize:13, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", color:"var(--c-red)", marginBottom:"0.5rem" }}>{S.negativeLabel}</div>
            <div style={{ fontSize:"1.5rem", color:"var(--c-text2)", lineHeight:1.5 }}>
          ...
          <Card gold style={{ flex:1 }}>
            <div style={{ fontSize:13, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", color:"var(--c-gold)", marginBottom:"0.5rem" }}>{S.positiveLabel}</div>
            <div style={{ fontSize:"1.5rem", color:"var(--c-text)", lineHeight:1.5 }}>
          ...
          {S.benefits.map((b, i) => (
            <Card key={i} green={b.highlight} gold={!b.highlight}>
              <div style={{ display:"flex", gap:"0.75rem", alignItems:"flex-start" }}>
                ...
                <div style={{ fontSize:"1.5rem", fontWeight:600, color:"var(--c-text)", lineHeight:1.4 }}>{b.title}</div>
              </div>
            </Card>
          ))}
        </div>
    ...
    <Slide key={4}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:"0.85rem", flex:1, maxHeight:"54vh" }}>
          {A.items.map((a, i) => (
            <Card key={i} gold style={{ display:"flex", flexDirection:"column", gap:"0.85rem" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <IconBox name={a.icon} gold size={26} />
                <span style={{ fontSize:14, fontWeight:700, color:"var(--c-gold)", opacity:0.4 }}>0{i+1}</span>
              </div>
              <div>
                <div style={{ fontSize:"1.85rem", fontWeight:700, color:"var(--c-text)", lineHeight:1.3 }}>{a.title}</div>
                {"desc" in a && a.desc && (
                  <div style={{ fontSize:"1.5rem", color:"var(--c-text2)", marginTop:"0.3rem", lineHeight:1.4 }}>{a.desc as string}</div>
                )}
              </div>
            </Card>
          ))}
    ...
    /* 5 — ПРОЕКТ */
    <Slide key={5}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"4rem 5rem 3rem" }}>
        <SlideLabel>{Pr.sectionLabel}</SlideLabel>
        <SlideTitle size="2.9rem">
          {Pr.title.split("\n")[0]} <GoldText>{Pr.title.split("\n")[1]}</GoldText>
        </SlideTitle>

        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1.1rem", flex:1 }}>
          {/* Фото */}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"0.7rem" }}>
            {Pr.geoItems.map((g, i) => (
              <Card key={i} gold style={{ display:"flex", gap:"0.75rem", alignItems:"flex-start" }}>
                <IconBox name={g.icon} gold size={24} />
                <div>
                  <div style={{ fontSize:"1.85rem", fontWeight:700, color:"var(--c-text)" }}>{g.title}</div>
                  <div style={{ fontSize:"1.5rem", color:"var(--c-text2)", lineHeight:1.45 }}>{g.text}</div>
                </div>
              </Card>
            ))}
            <Card gold>
              <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.12em", color:"var(--c-gold)", marginBottom:"0.45rem" }}>{Pr.flagshipBadge}</div>
              <div style={{ fontSize:"1.85rem", fontWeight:700, color:"var(--c-text)", marginBottom:"0.4rem" }}>{Pr.flagshipTitle}</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:"0.35rem" }}>
                {Pr.flagshipTags.map((tag, i) => (
                  <span key={i} style={{ fontSize:13, background:"rgba(201,168,76,0.1)", color:"var(--c-gold)", border:"1px solid rgba(201,168,76,0.22)", borderRadius:980, padding:"3px 10px" }}>{tag}</span>
                ))}
              </div>
            </Card>
          </div>
    ...
    <Slide key={6}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3rem", alignItems:"center" }}>
          {/* Левая */}
          <div>
            <div style={{ fontSize:"1.5rem", color:"var(--c-text3)", marginBottom:"0.5rem" }}>{Inv.cardLabel}</div>
            <div style={{ fontWeight:800, fontSize:"clamp(3.2rem, 7vw, 5.5rem)", letterSpacing:"-0.05em", lineHeight:1, background:"linear-gradient(135deg, var(--c-gold3) 0%, var(--c-gold) 50%, var(--c-gold2) 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", marginBottom:"1.75rem" }}>
              {Inv.bigNum}
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:"0.65rem", marginBottom:"1.5rem" }}>
              {Inv.details.map((d, i) => (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", borderBottom:"1px solid rgba(0,0,0,0.07)", paddingBottom:"0.65rem" }}>
                  <span style={{ fontSize:"1.5rem", color:"var(--c-text2)" }}>{d.label}</span>
                  <span style={{ fontSize:"1.5rem", fontWeight:700, color:"var(--c-text)" }}>{d.val}</span>
                </div>
              ))}
            </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"0.7rem" }}>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:"0.7rem" }}>
              {Inv.metrics.map((m, i) => (
                <Card key={i} gold style={{ textAlign:"center", padding:"1.1rem" }}>
                  <div style={{ color:"var(--c-gold)", marginBottom:6 }}>
                    <Icon name={m.icon as IconName} size={26} />
                  </div>
                  <div style={{ fontWeight:800, fontSize:"1.85rem", color:"var(--c-text)", letterSpacing:"-0.03em" }}>{m.val}</div>
                  <div style={{ fontSize:"1.2rem", color:"var(--c-text2)", marginTop:4 }}>{m.label}</div>
                </Card>
              ))}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:"0.7rem" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"0.7rem", padding:"0.85rem 1.1rem", borderRadius:14, background:"rgba(26,158,58,0.07)", border:"1px solid rgba(26,158,58,0.22)" }}>
              ...
              <span style={{ fontSize:"1.5rem", fontWeight:600, color:"var(--c-text)" }}>{Pr.contractsBadge}</span>
              <span style={{ fontSize:"1.2rem", color:"var(--c-text2)", marginLeft:"auto" }}>{Pr.contractsCol1Title}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <Slide key={7}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <SlideTitle size="2.9rem">{Con.title}</SlideTitle>

        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1.5rem", flex:1, maxHeight:"60vh" }}>
          {/* Контакты */}
          <div style={{ display:"flex", flexDirection:"column", gap:"0.75rem" }}>
            <a href="tel:89107600321" style={{ textDecoration:"none" }}>
              <Card gold style={{ display:"flex", alignItems:"center", gap:"1rem", cursor:"pointer" }}>
                <IconBox name="Phone" gold size={24} />
                <div>
                  <div style={{ fontSize:13, color:"var(--c-text3)", textTransform:"uppercase", letterSpacing:"0.1em" }}>{lang === "ru" ? "Позвонить" : "Call"}</div>
                  <div style={{ fontSize:"1.85rem", fontWeight:700, color:"var(--c-text)" }}>8 910 760 03 21</div>
                </div>
              </Card>
            </a>
            {[
              ...
              <Card key={i} style={{ display:"flex", gap:"0.85rem", alignItems:"center" }}>
                <IconBox name={c.icon} gold size={22} />
                <div>
                  <div style={{ fontSize:13, color:"var(--c-text3)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:2 }}>{c.label}</div>
                  <div style={{ fontSize:"1.5rem", fontWeight:600, color:"var(--c-text)" }}>{c.val}</div>
                </div>
              </Card>
            ))}
            <div style={{ marginTop:"auto" }}>
              <a href="tel:89107600321" style={{ display:"inline-flex", alignItems:"center", gap:8, background:"var(--c-gold)", color:"#000", borderRadius:980, padding:"14px 32px", fontWeight:700, fontSize:"1.5rem", textDecoration:"none", cursor:"pointer" }}>
