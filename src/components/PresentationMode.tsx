...
/* ── Заголовок слайда ── */
function SlideTitle({ children, size = "2.8rem" }: { children: React.ReactNode; size?: string }) {
  return (
    <h2 style={{
  ...
  const slides = [
    ...
    <Slide key={0}>
      ...
      <div style={{ position:"relative", zIndex:2, flex:1, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", textAlign:"center", padding:"0 4rem" }}>
        ...
        <h1 style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:800, fontSize:"clamp(2.4rem, 4.5vw, 4.2rem)", lineHeight:1.08, letterSpacing:"-0.04em", color:"var(--c-text)", margin:"0 0 1.5rem", maxWidth:960 }}>
          {T.hero.headline1}<br />
          {T.hero.headline2}<br />
          <GoldText>{T.hero.headlineAccent.replace("\n", " ")}</GoldText>
        </h1>

        <p style={{ fontSize:"clamp(1rem, 1.6vw, 1.2rem)", color:"var(--c-text2)", maxWidth:680, lineHeight:1.65, marginBottom:"3rem" }}>
          {T.hero.tagline}
        </p>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:"1px", background:"rgba(0,0,0,0.07)", borderRadius:20, overflow:"hidden", width:"100%", maxWidth:760 }}>
          {T.stats.map((s, i) => (
            <div key={i} style={{ background:"rgba(255,255,255,0.6)", padding:"1.25rem 1rem", textAlign:"center" }}>
              ...
              <div style={{ fontSize:13, color:"var(--c-text3)", marginTop:6 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <Slide key={1}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <SlideTitle size="2.4rem">
          {P.title1}<br />
          <span style={{ color:"var(--c-red)" }}>{P.title2}</span>
        </SlideTitle>

        <p style={{ fontSize:"1.05rem", color:"var(--c-text2)", lineHeight:1.6, marginBottom:"1.5rem", maxWidth:780 }}>
          {P.lead}
        </p>
          ...
          {P.items.map((item, i) => (
            <Card key={i} red>
              <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
                <span style={{ fontSize:11, fontWeight:700, color:"var(--c-red)", opacity:0.7, minWidth:22 }}>0{i+1}</span>
                <span style={{ fontSize:"1rem", fontWeight:600, color:"var(--c-text)", lineHeight:1.35 }}>{item}</span>
              </div>
            </Card>
          ))}
        </div>
    ...
    <Slide key={2}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <SlideTitle size="2.4rem">Кто выигрывает от наличия<br />собственного производства?</SlideTitle>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:"0.85rem", flex:1, maxHeight:"52vh" }}>
          {P.ambitions.map((a, i) => (
            <Card key={i} gold style={{ display:"flex", flexDirection:"column", gap:"0.6rem" }}>
              ...
              <div style={{ fontSize:"0.95rem", fontWeight:700, color:"var(--c-text)", lineHeight:1.35 }}>{a.title}</div>
              <div style={{ fontSize:"0.85rem", color:"var(--c-text2)", lineHeight:1.45 }}>{a.desc}</div>
            </Card>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <div style={{ display:"flex", gap:"0.85rem", marginBottom:"1.1rem" }}>
          <Card red style={{ flex:1 }}>
            <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", color:"var(--c-red)", marginBottom:"0.5rem" }}>{S.negativeLabel}</div>
            <div style={{ fontSize:"0.95rem", color:"var(--c-text2)", lineHeight:1.6 }}>
          ...
          <Card gold style={{ flex:1 }}>
            <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", color:"var(--c-gold)", marginBottom:"0.5rem" }}>{S.positiveLabel}</div>
            <div style={{ fontSize:"0.95rem", color:"var(--c-text)", lineHeight:1.6 }}>
          ...
          {S.benefits.map((b, i) => (
            <Card key={i} green={b.highlight} gold={!b.highlight}>
              <div style={{ display:"flex", gap:"0.75rem", alignItems:"flex-start" }}>
                ...
                <div style={{ fontSize:"0.9rem", fontWeight:600, color:"var(--c-text)", lineHeight:1.4 }}>{b.title}</div>
              </div>
            </Card>
          ))}
        </div>
          ...
          {A.items.map((a, i) => (
            <Card key={i} gold style={{ display:"flex", flexDirection:"column", gap:"0.85rem" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <IconBox name={a.icon} gold size={24} />
                <span style={{ fontSize:12, fontWeight:700, color:"var(--c-gold)", opacity:0.4 }}>0{i+1}</span>
              </div>
              <div>
                <div style={{ fontSize:"1rem", fontWeight:700, color:"var(--c-text)", lineHeight:1.4 }}>{a.title}</div>
                {"desc" in a && a.desc && (
                  <div style={{ fontSize:"0.88rem", color:"var(--c-text2)", marginTop:"0.3rem", lineHeight:1.4 }}>{a.desc as string}</div>
                )}
              </div>
            </Card>
          ))}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"0.7rem" }}>
            {Pr.geoItems.map((g, i) => (
              <Card key={i} gold style={{ display:"flex", gap:"0.75rem", alignItems:"flex-start" }}>
                <IconBox name={g.icon} gold size={22} />
                <div>
                  <div style={{ fontSize:"0.95rem", fontWeight:700, color:"var(--c-text)" }}>{g.title}</div>
                  <div style={{ fontSize:"0.88rem", color:"var(--c-text2)", lineHeight:1.45 }}>{g.text}</div>
                </div>
              </Card>
            ))}
            <Card gold>
              <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.12em", color:"var(--c-gold)", marginBottom:"0.45rem" }}>{Pr.flagshipBadge}</div>
              <div style={{ fontSize:"1rem", fontWeight:700, color:"var(--c-text)", marginBottom:"0.4rem" }}>{Pr.flagshipTitle}</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:"0.35rem" }}>
                {Pr.flagshipTags.map((tag, i) => (
                  <span key={i} style={{ fontSize:11, background:"rgba(201,168,76,0.1)", color:"var(--c-gold)", border:"1px solid rgba(201,168,76,0.22)", borderRadius:980, padding:"3px 10px" }}>{tag}</span>
                ))}
              </div>
            </Card>
          </div>
    ...
    <Slide key={6}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3rem", alignItems:"center" }}>
          {/* Левая */}
          <div>
            <div style={{ fontSize:"0.9rem", color:"var(--c-text3)", marginBottom:"0.5rem" }}>{Inv.cardLabel}</div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"0.65rem", marginBottom:"1.5rem" }}>
              {Inv.details.map((d, i) => (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", borderBottom:"1px solid rgba(255,255,255,0.07)", paddingBottom:"0.65rem" }}>
                  <span style={{ fontSize:"0.95rem", color:"var(--c-text2)" }}>{d.label}</span>
                  <span style={{ fontSize:"0.95rem", fontWeight:700, color:"var(--c-text)" }}>{d.val}</span>
                </div>
              ))}
            </div>
            <div style={{ fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.14em", color:"var(--c-gold)", opacity:0.75, marginBottom:"0.65rem" }}>{Pr.contractsCol2Title}</div>
            <div style={{ display:"flex", gap:"0.6rem" }}>
              {Pr.contractsPartners.map((p, i) => (
                <div key={i} style={{ flex:1, padding:"0.75rem 1rem", borderRadius:14, background:"rgba(201,168,76,0.07)", border:"1px solid rgba(201,168,76,0.15)" }}>
                  <div style={{ fontSize:"1rem", fontWeight:700, color:"var(--c-text)" }}>{p.name}</div>
                  <div style={{ fontSize:"0.82rem", color:"var(--c-text2)", lineHeight:1.4, marginTop:3 }}>{p.desc}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {Inv.metrics.map((m, i) => (
                <Card key={i} gold style={{ textAlign:"center", padding:"1.1rem" }}>
                  <div style={{ color:"var(--c-gold)", marginBottom:6 }}>
                    <Icon name={m.icon as IconName} size={24} />
                  </div>
                  <div style={{ fontWeight:800, fontSize:"1.6rem", color:"var(--c-text)", letterSpacing:"-0.03em" }}>{m.val}</div>
                  <div style={{ fontSize:"0.8rem", color:"var(--c-text2)", marginTop:4 }}>{m.label}</div>
                </Card>
              ))}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:"0.7rem" }}>
              {Inv.infraItems.map((it, i) => (
                <Card key={i} style={{ textAlign:"center", padding:"1rem" }}>
                  ...
                  <div style={{ fontWeight:700, fontSize:"0.95rem", color:"var(--c-text)" }}>{it.val}</div>
                  <div style={{ fontSize:"0.8rem", color:"var(--c-text2)", marginTop:3, lineHeight:1.3 }}>{it.label}</div>
                </Card>
              ))}
            </div>
    ...
    <Slide key={7}>
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"4rem 5rem 3rem" }}>
        ...
        <SlideTitle size="2.4rem">{Con.title}</SlideTitle>
            ...
            <a href="tel:89107600321" style={{ textDecoration:"none" }}>
              <Card gold style={{ display:"flex", alignItems:"center", gap:"1rem", cursor:"pointer" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:"1.4rem", fontWeight:700, color:"var(--c-text)" }}>8 910 760 03 21</div>
                </div>
              </Card>
            </a>
            {[
              ...
              <Card key={i} style={{ display:"flex", gap:"0.85rem", alignItems:"center" }}>
                ...
                <div>
                  <div style={{ fontSize:11, color:"var(--c-text3)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:2 }}>{c.label}</div>
                  <div style={{ fontSize:"1rem", fontWeight:600, color:"var(--c-text)" }}>{c.val}</div>
                </div>
              </Card>
            ))}
            <div style={{ marginTop:"auto" }}>
              <a href="tel:89107600321" style={{ display:"inline-flex", alignItems:"center", gap:8, background:"var(--c-gold)", color:"#000", borderRadius:980, padding:"12px 28px", fontWeight:700, fontSize:"1rem", textDecoration:"none", cursor:"pointer" }}>
